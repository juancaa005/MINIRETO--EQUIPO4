    ...
    private void colocarBarcosJugador() {
        ...
        for (int tipo = 0; tipo < TAMANIO_BARCOS.length; tipo++) {
            ...
            for (int i = 0; i < CANTIDAD_BARCOS[tipo]; i++) {
                ...
                while (!colocado) {
                    mostrarTableroJugadorLado(true); // Mostrar los barcos durante la colocación
                    ...
                    if (esColocacionValida(tableroJugador, fila, columna, tamaño, horizontal)) {
                        // Colocar el barco
                        colocarBarco(tableroJugador, barcosJugador, barcoId, fila, columna, tamaño, horizontal);
                        barcoId++;
                        totalBarcosJugador++;
                        colocado = true;
    ...
    private boolean esColocacionValida(int[][] tablero, int fila, int columna, 
        ...
        if (horizontal) {
            ...
            for (int c = columna; c < columna + tamaño; c++) {
                if (tablero[fila][c] != AGUA_OCULTA) return false;
            }
        } else {
            ...
            for (int f = fila; f < fila + tamaño; f++) {
                if (tablero[f][columna] != AGUA_OCULTA) return false;
            }
        }
        return true;
    }
